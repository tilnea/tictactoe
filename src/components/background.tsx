...
interface BackgroundProps extends WrapperProps {
  children: ReactNode;
}
...
const Wrapper = styled.div<WrapperProps>`
  ...
  background-image: url(${({ $player }) =>
      $player === Player.X || $player === Player.NO ? xSVG : oSVG}),
    url(${({ $player }) =>
      $player === Player.O || $player === Player.NO ? oSVG : xSVG});
  background-position: -30px, 30px;
  height: 100vh;
  width: 100vw;
`;

export const Background = ({ children, $player }: BackgroundProps) => {
  return <Wrapper $player={$player}>{children}</Wrapper>;
};
